...
| `public/models/cars/pickup.glb` — the Ford F150 | **This repository's own work.** Built by `tools/make-truck.mjs` from boxes and cylinders; nothing downloaded, no third-party geometry. | Authored because every CC0 pickup available (Poly Pizza: Pickup Truck, Armored, Heart, Thunder, Crate and five more) is a single-material TEXTURE-ATLAS model, and this game paints a car by reading separated material names. The rest were CC-BY, which is off the allowed list. "Ford" and "F-150" are Ford's trademarks; the geometry is a generic full-size pickup and the label lives in one place (`game/garage.js`). |
