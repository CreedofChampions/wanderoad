 ...
 *  crawls under it is a chauffeur who quietly cancels your run. */
const CRAWL = 9;
...
export class Autopilot {
  constructor({ cruise = 16 } = {}) {
    ...
    this.tel = { lateral: 0, err: 0, kappa: 0, kMax: 0, target: 0, delta: 0, maxA: 0, hops: 0, road: '' };
  }
  ...
  update(car, manual, dt) {
    ...
    const target = Math.max(Math.min(this.cruise, kMax > 1e-5 ? Math.sqrt(CORNER_G / kMax) : Infinity), CRAWL);
 ...
 * to stand a controller up or re-derive arc length from a polyline. */
export { pathOf, headingAt, locate };
