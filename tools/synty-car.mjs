...
const [IN, OUT, ATLAS] = [process.argv[2], process.argv[3], process.argv[4]];
if (!IN || !OUT) {
...
/* ── THE ATLAS, BAKED INTO VERTEX COLOURS ───────────────────────────────
 *
 * Operator: "also no textures on the cars".
 *
 * He is right, and the first version of this file caused it. A Synty vehicle is ONE body mesh whose
 * detail — lights, grille, bumpers, interior, panel lines — lives entirely in a 2048² palette atlas,
 * and this tool was throwing the TEXCOORDs away and painting the whole body one flat colour. That is
 * fine for a Quaternius model, which separates its parts by material; it is exactly wrong for Synty,
 * which separates them by UV.
 *
 * The answer is not to ship the texture and teach the painted shader to sample it. This game's whole
 * look is one untextured painted material, and adding a sampler for one pack would fork the pipeline.
 * A Synty atlas is a PALETTE — flat blocks of colour, not detail — so sampling it ONCE PER VERTEX at
 * conversion time and writing the result into the vertex colour is very nearly lossless, and it lands
 * the model in exactly the form the painted shader already wants.
 *
 * Nothing is downloaded at runtime, no texture ships, and the atlas never leaves this machine.
 */
...
/** sRGB byte -> linear, the same conversion core/palette.js uses. The painted shader works linear. */
const toLinear = (v) => {
  const f = v / 255;
...
const atlas = ATLAS ? readPng(readFileSync(ATLAS)) : null;
...
(gltf.meshes || []).forEach((m, mi) => {
  ...
  for (const prim of m.primitives) {
    ...
    /* One atlas sample per vertex, written as COLOR_0. `classify` in car/loadedCar.js still decides
     * glass, wheels and lights from the material name — this only supplies the BODY's own colours,
     * which is the detail Synty puts in the texture and nowhere else. */
    let col = null;
    if (atlas && UV) {
      ...
      for (let v = 0; v < P.length / 3; v++) {
        ...
        // glTF UV origin is top-left; a PNG's rows run the same way, so no flip is needed here.
        const ay = Math.min(atlas.h - 1, Math.max(0, Math.round(vv * (atlas.h - 1))));
        const i = (ay * atlas.w + ax) * atlas.ch;
        col[v * 3] = toLinear(atlas.px[i]);
        col[v * 3 + 1] = toLinear(atlas.px[i + 1]);
        col[v * 3 + 2] = toLinear(atlas.px[i + 2]);
      }
    }
    ...
    if (col) accessors.push({ bufferView: vc, componentType: 5126, count: col.length / 3, type: 'VEC3' });
    accessors.push({
      bufferView: vi,
    ...
    const attrs = col
      ? { POSITION: accessors.length - 4, NORMAL: accessors.length - 3, COLOR_0: accessors.length - 2 }
      : { POSITION: accessors.length - 3, NORMAL: accessors.length - 2 };
    meshesOut.push({
      name: n > 1 ? `${r.node}_${n}` : r.node,
      primitives: [{ attributes: attrs, indices: iIdx, material: MATERIALS.indexOf(r.mat) }],
    });
  }
});
